...
Some of you had issues with the `groundhog` package. If that's you: please forget about the `groundhog` package. As long as you have the `tidyverse` and `here` packages installed (and your versions aren't really old) -- you should be fine. Try uncommenting and running the code below if you're (still) having issues.
...
We started the coding demo on day 1 with a function -- `getwd()`. The five lines under "# how to learn about your variables" in the code chunk above are also functions.
...
Data frames are the most common data structure you'll encounter doing real data analysis in R -- think of it as the R version of an Excel table. It is similar to a matrix in that it also has row and column attributes, but different in that its columns, for example, can be of multiple data types (only 1 for matrices).
...
Fun fact: the pipe operator is possible because the functions we will learn in the `tidyverse` are standardized to take data frames as the first argument (input variable). (Confirm this by typing `?select`, what's the first argument)
...
First, save the `penguins` dataset as a tibble to a variable name of your choosing.
